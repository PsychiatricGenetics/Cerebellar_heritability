setwd("C:/GitHub/Cerebellar_heritability/sfigure2")
...
for (i in list1){
  rMat <- read.csv(paste0("5-qtim_bivariate_AE_", i, "Z_output.csv"))
...
# Row/col order
rc.order <- read.table("qtim_bivariate_AE_matrices_order.txt", sep = '\t', header = T)
...
diag(rV_matrix) <- 1
write.csv(rV_matrix, "qtim_bivariate_AE_rPH_matrix.csv")
...
#### Plots ####
pdf(file = "SFigure_2_prelim.pdf", width = 20, height = 20)
par(mfrow=c(1,2))
