...
source("1-combined_univariate_function.R")
...
result.single %>% select(Variable, ACE_A_95CI, ACE_C_95CI, ACE_E_95CI, ACE_VA, ACE_VC, ACE_VE, AE_A_95CI, AE_E_95CI, AE_A, AE_E)
...
result.multiple.ACE <- lapply(variable.listZ, Univariate_ACDE, twin.data = my.data, covariate = c("Sex", "Age", "eTIVZ", "cohort"), CorD = "C") %>% bind_rows()
write.csv(result.multiple.ACE, "1-combined_univariate_ACE_output.csv", row.names = F)

#### ADE ####
result.multiple.ADE <- lapply(variable.listZ, Univariate_ACDE, twin.data = my.data, covariate = c("Sex", "Age", "eTIVZ", "cohort"), CorD = "D") %>% bind_rows()
...
result.multiple.ADE <- 
  result.multiple.ADE %>% rename("ADE_A_95CI"="ACE_A_95CI",
                               ...
                               "dropD"="dropC",
                               "dropAD"="dropAC")
write.csv(result.multiple.ADE, "1-combined_univariate_ADE_output.csv", row.names = F)
