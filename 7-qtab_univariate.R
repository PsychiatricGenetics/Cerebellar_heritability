...
source("7-qtab_univariate_function.R")

my.data <- readRDS("data/qtab_cb_familywise.RDS")

# Single variable
result.single <- as_tibble(Univariate_ACDE(phenotype = "Right_VIZ", twin.data = my.data, covariate = c("Sex", "Age", "eTIVZ"), CorD = "C"))
...
write.csv(result.multiple.ACE, "7-qtab_univariate_ACE_output.csv", row.names = F)
...
result.multiple.ADE <- 
  result.multiple.ADE %>% rename("ADE_A_95CI"="ACE_A_95CI",
                                 ...
                                 "dropD"="dropC",
                                 "dropAD"="dropAC")
write.csv(result.multiple.ADE, "7-qtab_univariate_ADE_output.csv", row.names = F)
