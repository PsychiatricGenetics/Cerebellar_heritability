...
library(stringr)
library(dplyr)
twin.data <- readRDS("data/qtab_cb_familywise.RDS")
source("miFunctions.R")
source("6-qtab_saturated_function.R")

# Run for single phenotype
Saturated_covariate(phenotype = "Left_Crus_IZ", twin.data = twin.data)
...
# Save Output
write.csv(results.sat, "6-qtab_saturated_output.csv", row.names = F)
