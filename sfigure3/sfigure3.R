setwd("C:/GitHub/Cerebellar_heritability/sfigure3")
...
for (i in list1){
  rMat <- read.csv(paste0("8-qtab_bivariate_AE_", i, "Z_output.csv"))
...
#### Check for differences between model orders ####
rV.2 <-
...
# Row/col order
rc.order <- read.table("qtab_bivariate_AE_matrices_order.txt", sep = '\t', header = T)
...
diag(rV_matrix) <- 1
write.csv(rV_matrix, "qtab_bivariate_AE_rPH_matrix.csv")
...
rA_E_matrix[upper.tri(rA_E_matrix)] <- rE_matrix[upper.tri(rE_matrix)] # Upper tri of the rE matrix
...
#### Plots ####
pdf(file = "Figure_S3_prelim.pdf", width = 20, height = 20)
par(mfrow=c(1,2))
