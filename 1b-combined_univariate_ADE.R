...
# Single phenotype
result.single <- as_tibble(Univariate_ADE(phenotype = "Left_I_IIIZ", twin.data = my.data, covariate = c("Sex", "Age", "eTIVZ", "cohort")))
...
result.multiple <- lapply(phenotype.arrayZ, Univariate_ADE, twin.data = my.data, covariate = c("Sex", "Age", "eTIVZ", "cohort")) %>% bind_rows()
result.multiple <- as_tibble(result.multiple)

write.csv(result.multiple, "combined_univariate_ADE_output.csv", row.names = F)
